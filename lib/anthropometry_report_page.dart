...
class _AnthropometryReportPageState extends State<AnthropometryReportPage> {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('anthropometry').doc(docId).delete();
  ...
  Future<void> _deleteBulk() async {
    ...
    for (final id in _selectedIds) {
      batch.delete(FirebaseFirestore.instance.collection('anthropometry').doc(id));
    }
    await batch.commit();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Anthropometry Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
                            ...
                            onSelected: (value) {
                              if (value == 'edit') {
                                Navigator.push(context, MaterialPageRoute(
                                  builder: (_) => AnthropometryPage(existingData: data, docId: doc.id),
                                ));
