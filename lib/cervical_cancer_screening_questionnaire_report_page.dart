  ...
  void _deleteRecord(String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this screening entry?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('cervical_screening').doc(docId).delete();
          ...
          onSelected: (value) {
            if (value == 'edit') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => CervicalCancerScreeningPage(
                    existingData: record,
                    docId: doc.id,
                  ),
                ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: _isSearchingActive
            ...
            : const Text('Cervical Screening Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('cervical_screening').snapshots(includeMetadataChanges: true),
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            if (data['Exam_Date'] != null)
                              Text('Date: ${data['Exam_Date'] is Timestamp ? DateFormat('dd-MMM-yyyy').format((data['Exam_Date'] as Timestamp).toDate()) : data['Exam_Date'].toString()}'),
                            Text('Result: ${data['VIA_Examination_Result'] ?? 'N/A'}'),
                          ],
                        ),
