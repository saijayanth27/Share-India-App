...
class CervicalCancerScreeningReportPage extends StatefulWidget {
  const CervicalCancerScreeningReportPage({super.key});
  @override
  State<CervicalCancerScreeningReportPage> createState() => _CervicalCancerScreeningReportPageState();
}

class _CervicalCancerScreeningReportPageState extends State<CervicalCancerScreeningReportPage> {
  ...
  void initState() {
    super.initState();
    _reportStream = FirebaseFirestore.instance.collection('cervical_screening').snapshots(includeMetadataChanges: true);
  ...
  void _deleteRecord(String docId) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () { FirebaseFirestore.instance.collection('cervical_screening').doc(docId).delete(); Navigator.pop(context); },
  ...
  Future<void> _deleteBulk() async {
    ...
    for (final id in _selectedIds) {
      batch.delete(FirebaseFirestore.instance.collection('cervical_screening').doc(id));
    }
    await batch.commit();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Cervical Screening Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true, elevation: 0, backgroundColor: Colors.transparent,
        flexibleSpace: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.green.shade700, Colors.teal.shade400], begin: Alignment.topLeft, end: Alignment.bottomRight))),
      ),
      ...
      return Card(
        ...
        child: ListTile(
          ...
          trailing: PopupMenuButton<String>(
            ...
            onSelected: (v) { if (v == 'edit') Navigator.push(context, MaterialPageRoute(builder: (_) => CervicalCancerScreeningPage(existingData: data, docId: doc.id))); else if (v == 'delete') _deleteRecord(doc.id); },
            itemBuilder: (_) => [
