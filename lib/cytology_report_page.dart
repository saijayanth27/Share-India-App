...
class CytologyReportPage extends StatefulWidget {
  const CytologyReportPage({super.key});
  @override
  State<CytologyReportPage> createState() => _CytologyReportPageState();
}

class _CytologyReportPageState extends State<CytologyReportPage> {
  ...
  void initState() {
    super.initState();
    _reportStream = FirebaseFirestore.instance.collection('cytology').snapshots(includeMetadataChanges: true);
  ...
  void _deleteRecord(String docId) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () { FirebaseFirestore.instance.collection('cytology').doc(docId).delete(); Navigator.pop(context); },
  ...
  Future<void> _deleteBulk() async {
    ...
    for (final id in _selectedIds) {
      batch.delete(FirebaseFirestore.instance.collection('cytology').doc(id));
    }
    await batch.commit();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Cytology Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true, elevation: 0, backgroundColor: Colors.transparent,
        flexibleSpace: Container(decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.pink.shade700, Colors.pinkAccent.shade200], begin: Alignment.topLeft, end: Alignment.bottomRight))),
      ),
      ...
      return Card(
        ...
        child: ListTile(
          ...
          trailing: PopupMenuButton<String>(
            ...
            onSelected: (v) { if (v == 'edit') Navigator.push(context, MaterialPageRoute(builder: (_) => CytologyPage(existingData: data, docId: doc.id))); else if (v == 'delete') _deleteRecord(doc.id); },
            itemBuilder: (_) => [
