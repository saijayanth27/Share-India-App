...
class _EyeExaminationReportPageState extends State<EyeExaminationReportPage> {
  ...
  final Map<String, String> _fieldMapping = {
    ...
    'Interviewer': 'Interviewer_s_Name',
    'OD Symptoms': 'Signs_and_symptoms',
    'OS Symptoms': 'Signs_and_symptoms2',
  };
  ...
  void _deleteRecord(String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this eye examination entry?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('eye_examination').doc(docId).delete();
          ...
          onSelected: (value) {
            if (value == 'edit') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => EyeExaminationPage(
                    existingData: record,
                    docId: doc.id,
                  ),
                ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: _isSearchingActive
            ...
            : const Text('Eye Examination Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('eye_examination').snapshots(includeMetadataChanges: true),
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            if (data['Examination_Date'] != null)
                              Text('Date: ${data['Examination_Date'] is Timestamp ? DateFormat('dd-MMM-yyyy').format((data['Examination_Date'] as Timestamp).toDate()) : data['Examination_Date'].toString()}'),
                            Text('OD Symptoms: ${data['Signs_and_symptoms'] ?? 'N/A'}'),
                          ],
                        ),
