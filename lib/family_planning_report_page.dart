...
class _FamilyPlanningReportPageState extends State<FamilyPlanningReportPage> {
  ...
  Future<void> _deleteBulk() async {
    ...
    for (final id in _selectedIds) {
      batch.delete(FirebaseFirestore.instance.collection('family_planning').doc(id));
    }
    await batch.commit();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Family Planning Report', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
                            ...
                            onSelected: (value) {
                              if (value == 'edit') {
                                Navigator.push(context, MaterialPageRoute(
                                  builder: (_) => FamilyPlanningPage(existingData: data, docId: doc.id),
                                ));
