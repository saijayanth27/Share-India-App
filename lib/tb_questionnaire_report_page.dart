...
class _TBQuestionnaireReportPageState extends State<TBQuestionnaireReportPage> {
  ...
  final Map<String, String> _fieldMapping = {
    ...
    'Reason (if not)': 'If_not_done_reason',
  };
  ...
  void _deleteRecord(String docId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to delete this TB questionnaire entry?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('tb_questionnaire').doc(docId).delete();
          ...
          onSelected: (value) {
            if (value == 'edit') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => TBQuestionnairePage(
                    existingData: record,
                    docId: doc.id,
                  ),
                ),
    ...
    return Scaffold(
      appBar: AppBar(
        title: _isSearchingActive
            ...
            : const Text('TB Questionnaire Reports', style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        elevation: 0,
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('tb_questionnaire').snapshots(includeMetadataChanges: true),
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            if (data['Date_of_Interview'] != null)
                              Text('Interview Date: ${data['Date_of_Interview'] is Timestamp ? DateFormat('dd-MMM-yyyy').format((data['Date_of_Interview'] as Timestamp).toDate()) : data['Date_of_Interview'].toString()}'),
                          ],
                        ),
